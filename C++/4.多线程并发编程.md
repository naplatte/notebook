...
- 用处：和常见的互斥锁mutex一样，都是用来保护临界资源，确保同一时间是由一个线程访问共享数据
- 区别在于
  - **自旋锁等待资源的方式是忙等** - 等待时持续消耗CPU资源
  - 互斥锁等待资源的方式是休眠/阻塞 - 等待时不消耗CPU资源
- 忙等的优势：不需要切换进程/线程状态 - 不需要内核态的参与 - 不用频繁变态
- 休眠/阻塞的优势当然显而易见了
- 自旋锁的适用场景：临界区**很短**（通常只有几条指令） - 变态有点不值当的了 - 避免线程线上下文切换
  - 如果临界区很长的话，一直占用CPU忙等就浪费了资源
- 自旋何意味：**在一个小循环中不断重复尝试获取锁**，就像原地自旋一样，直到等到锁
...
- 关于`std::atomic_flag`
  - 是<atomic>库中提供的一个**最简单、最原始**的原子类型
  - 其只有两个public成员函数，分别为`test_and_set()`和`clear()`
    - `test_and_set()`：1.读取当前标志的值 2.将标志的值设为true，这两个操作都是原子的操作（原来真，返回真 - 循环等待； 原来假，返回真 - 上锁）
    - `clear()`：原子的将标志设为False，一般初始化标识时使用
- 实现自旋锁 -  test_and_set就自带自旋锁属性，无需多言了 - 加个while循环一直尝试就行了
...
class Spinlock {
private:
    std::atomic_flag flag = ATOMIC_FLAG_INIT; // 必须使用这个宏初始化

public:
    // 尝试获取锁
    void lock() {
        // test_and_set() 返回 false 表示锁获取成功
        // 因为它返回了设置之前的值（即 Clear 状态）
        while (flag.test_and_set(std::memory_order_acquire)) {
            // 循环在这里“自旋”，直到 test_and_set() 返回 false
            // 表示成功将标志从 Clear 切换为 Set
        }
    }

    // 释放锁
    void unlock() {
        // 将标志设置为 Clear，让其他线程可以获取锁
