...
- 一个装固定大小的数组容器，是C风格数组的安全替代品
- 特点
  - 大小在编译时就确定，固定
  - 支持stl容器的接口（size(),begin(),end()）等
  - 内部连续存储，和普通数组一样高效（只是逻辑地址连续，物理地址不一定连续）
  - 不能动态分配内存（静态数组）
...
| 功能           | 方法                        | 示例                                                         |
| -------------- | --------------------------- | ------------------------------------------------------------ |
| 初始化         | `array<type,n> arr`         | `std::array<int,5> arr = {1,2,2,3,4};`                       |
| 获取大小       | `arr.size()`                | `std::cout << arr.size();`                                   |
| 访问元素       | `arr[i]` 或 `arr.at(i)`     | `arr[2] = 10;`  `arr.at(2) = 10; // 会检查越界`              |
| 遍历           | `for` 或 `range-based for`  | `for(auto x : arr) std::cout << x;`                          |
| 获取首尾迭代器 | `arr.begin()`, `arr.end()`  | `for(auto it = arr.begin(); it != arr.end(); ++it) std::cout << *it;` |
| 访问首尾元素   | `arr.front()`, `arr.back()` | `arr.front() = 100; arr.back() = 200;`                       |
| 填充           | `arr.fill(value)`           | `arr.fill(0);`                                               |
